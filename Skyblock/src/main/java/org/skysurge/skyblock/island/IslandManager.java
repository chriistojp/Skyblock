package org.skysurge.skyblock.island;
...
public class IslandManager {
    ...
    public static void createIsland(Player p) {
        ...
        s.createSection("members");

        saveIslands();
    }

    public static void islandInvite(Player owner, Player member) {
        Player invited = member;
        if(islandConfig.contains(owner.getUniqueId().toString())) {
            owner.sendMessage("You have successfully invited " + invited.getName());
            invited.sendMessage("You have been invited to join " + owner.getName() + "'s island. Type /island accpet to join them.");
            invites.put(invited.getUniqueId(), owner.getUniqueId());
        } else {
            owner.sendMessage("Sorry, you are not the island leader.");
        }
    }

    public static void islandAccept(Player owner, Player member) {
        List<String> members = islandConfig.getStringList(owner.getUniqueId() + ".members");
        members.add(member.toString());
        ConfigurationSection s = islandConfig.getConfigurationSection(owner.getUniqueId() + "members");
        islandConfig.set(owner.getUniqueId() + ".members", members);
        owner.sendMessage(islandConfig.get(owner.getUniqueId() + ".members").toString());
        member.sendMessage("You have successfuly joined " + owner.getName() + "'s island!");
    }
